...
{
    public class TaxRepository : Base.CompositeRepositoryBase<Tax, int, Guid, TaxBody, TaxParameters>, ITaxRepository
    {
        public TaxRepository(RepositoryContext repositoryContext, SemaphoreSlim globalRepositorySemaphore, AppSettings appSettings, ILogger logger, IAppState appState) : base(repositoryContext, globalRepositorySemaphore, appSettings, logger, appState)
        {
            EndpointEntity = "Tax";
        }
        ...
        {
            return JsonConvert.DeserializeObject<(int, Guid)>(id);
        }
    }
}
