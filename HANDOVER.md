...
**▶ NEXT — T1.7 standing suites** (spec §T1.7, permanent): (1) tenant isolation — enumerate all `/api/v1` routes via ApiExplorer, as tenant A attempt tenant-B resource IDs → expect 404/empty, never 200-with-data or 403 (existence leak); (2) money reconciliation — random valid sale batches → ingest → Σ DB == Σ submitted in pence; (3) idempotency replay — capture 200 ingests, replay twice → row counts unchanged. Also fold in the deferred HTTP-level e2e + 20-way concurrent ingest (needs a real MySQL or Testcontainers). Verify a deliberately-omitted filter is caught by suite 1.
...
**Phase 0 + Phase 1 T1.1-T1.6 COMPLETE** (tenancy schema + row-level scoping + provisioning/enrolment/device-token API + scope-based auth; all migrations applied to staging `plutus_t1` only; live `plutus`/ETRIE untouched). All pushed to `github.com/LT-Mhuggett/Plutus` `Matt's-Horror` (latest `d2915fd`). 33/33 unit + 5/5 arch green. Live test env healthy. Resume at §5 — Phase 1 **T1.7** (standing test suites).
