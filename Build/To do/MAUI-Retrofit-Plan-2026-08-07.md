...
4. ⚠ **For WP6–13, [`MAUI-Cutover-Plan-2026-08-09.md`](MAUI-Cutover-Plan-2026-08-09.md) is the
   execution order and carries binding defaults 10–18.** This plan keeps the WP bodies and DoDs;
   that plan sequences them into 28 dependency-ordered steps and holds the decisions the survey
   surfaced. Its §A protocol extends this §0. **Work WP6–13 from there, one step per session.**
...
| **4** Enrolment + device identity | ✅ | **Placement closed 2026-08-09** (cutover step 4): enrolment runs through `Client.Storage.EnrolmentFlow`, so the till records TillId/StoreId/**BusinessId**/TenantId/ServerUrl instead of only a credential, and `TillPlacement` refreshes it on every start (a till can be moved between stores; it also self-heals tills enrolled before this). ⚠ Archive gate still OFF until cutover step 21 gives the app a way to archive. · 2026-08-07 · `f90dac5`. Server URL + code; **archive gate refuses enrolment** while a legacy DB is un-archived; placement (storeId + legacy businessId) refreshed each start; secret asserted absent from the DB file. |
| **5** Heartbeat + catalogue sync · **5b** | ✅ | **The backend gap is CLOSED (2026-08-08).** Built + tested headlessly: `POST /api/v1/heartbeat` (in-process `TillPresence`, 2/5-min boundaries), `GET /api/v1/catalogue/changes` (keyset cursor, tombstones), `SyncNow`/`Locked`/`LockReason` on `Device` (migration `AddDeviceSyncSignals`, **carries an index** — see below), and the client half in `Client.Core/SyncClient.cs` + `TillStore : ISyncStore`. 22 new tests. **Closed 2026-08-08:** the price-schedule half of the feed (server now delegates to `SharedKernel.PriceResolution` rather than holding a second copy), MAUI's VAT-band consumption via `VatBandCache`, and the search matcher — which went from **three** implementations to one shared `SharedKernel.ItemSearch`. **Closed 2026-08-09 (`ea9787f`):** the catalogue sync is actually CALLED — `CatalogueSyncService` runs on sign-in (background) and from the Plutus tab, which now performs the real sync instead of a five-item diagnostic that stored nothing. ⚠ Remaining: the **60s scheduler + outbox drain** (`OutboxPusher.DrainAsync` is still referenced nowhere) — cutover plan step 13. |
...
| 6–7, 9–13 | ⬜ | ⚠ **Execute from [`MAUI-Cutover-Plan-2026-08-09.md`](MAUI-Cutover-Plan-2026-08-09.md)** — its steps 20–27 sequence these WPs behind the money path they all depend on. ⚠ The old note here ("they need a device to verify") was **wrong**: `AppClient.Tests` has `InternalsVisibleTo` from the app, so viewmodel/service logic is headlessly testable; only XAML, printer, drawer and scanner need hardware. WP7, WP10 and WP12 each gained real scope on 2026-08-08 (pushed theming · add-unknown + Bin/untracked · member-number scan). |
| **14** Payment-gateway awareness | 🔨 | Added 2026-08-08 (was cited in Part B as "WP17.2", which is not a work package in this plan). **Shared half done 2026-08-09** — `Client.Core.PaymentGateway.Resolve`, 13 tests, mutation-checked. ⚠ The rule is **`integrated`, not `provider`**: every provider reports `integrated: false` today, so a till reading the name would wait for a terminal that never answers. **Remaining:** the MAUI checkout XAML (needs a device). |
...
>    nothing else.
>    ✅ **SUPERSEDED 2026-08-09 (`ea9787f`):** the EF wall came down — `Plutus/Data/Database` was
>    retargeted to `net10.0`/EF 9 (the 3.1-compiled migrations were the real break, pinned by
>    `LegacyDatabaseUnderEf9Tests`) and AppClient now references `Plutus.Client.Storage`. The JSON
>    roster file's stated blocker is gone; it moves onto `TillDbContext.Operators` in cutover plan
>    step 24.
...
compromised till. **Now binding default 12** (cutover plan §B): quarantine 202, never a 400 —
✅ **Confirmed by Matt 2026-08-09: "You should not be able to refund MORE than the price paid for
it"** — enforced at both gates, no override past the remainder; cutover §B default 12 has the full
ruling.
...
| 9 | ⚠ **NO BRIDGE TO THE LEGACY LOCAL DATABASE. The MAUI screens cut over to `Plutus.Client.Storage`.** ⚠ **The ordered plan for this now lives in [`MAUI-Cutover-Plan-2026-08-09.md`](MAUI-Cutover-Plan-2026-08-09.md) — read it before starting WP6–13.** It was written from a survey of the tree and it corrects this plan in two ways that matter: the work is **11–15 weeks**, not the tail of a retrofit; and **the MAUI till cannot post a sale to Plutus at all today**, so every screen port is cosmetic until the money path (its steps 9–14) lands. It also shows "WP6–13 need a device" to be substantially false — viewmodel and service logic is headlessly testable now. *(Matt, 2026-08-09: "I would not bridge the legacy DB, it is not needed.")* The question arose the moment the till opened and its search found nothing: MAUI's screens read the legacy `Database.db` (`Items`, `Stores`, `Employees`), while WP2/WP3/WP5 put the synced catalogue, the outbox and committed sales in the **v2** store — two different databases. Writing synced items back into the legacy tables would have lit every screen up in an afternoon, and left every till carrying two copies of its catalogue and a schema nobody intends to keep. **Decided against.** ⚠ The consequence is honest and worth stating: a screen shows nothing until it is ported, so the till gets *more* visibly incomplete before it gets better. Scope, measured 2026-08-09: **37 call sites across 21 files**, concentrated in Till, Inventory, Statistics, StoreOptions, FirstTimeStartUp and Login. | WP2, WP6–13 |
| 10–18 | ⚠ **Defaults 10–18 live in [`MAUI-Cutover-Plan-2026-08-09.md`](MAUI-Cutover-Plan-2026-08-09.md) §B** — added 2026-08-09 to make WP6–13 executable with no questions. Headlines: **10** when in doubt, match the web till · **11** online operator auth = `POST /api/Auth/Login` like the web till (plus the `Employee.Active` fix) · **12** ✅ refund cap CONFIRMED by Matt — never more than was paid, enforced at both gates, quarantine 202 at ingest · **13** ✅ tenders CONFIRMED — the fixed five-value `TenderType` set suffices, a sixth is additive · **14** manual line discounts first · **15** parked baskets local-only, contract JSON · **16** ⚠ **first sign-in of any account must be ONLINE** *(Matt, 2026-08-09)* — mints a device-local verifier; hash-shipping retires once both tills carry verifiers · **17** report series with no server answer are dropped, never locally recomputed · **18** additive catalogue-feed fields allowed (Brand/Desc/Cost/StockQty/Barcodes); the store-info logo is dropped. | WP6–13, WP8/16 |
