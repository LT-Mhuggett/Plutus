...
|---|---|---|
| **Backend** | **1.22.0** | ✅ **DEPLOYED 2026-08-21 (late)** — the supervisor-and-above item gate, the "Add/edit stock" role, the item-History stock merge, and WP10's two widened endpoint gates. ⚠ **NO MIGRATION** in 1.21.0 or 1.22.0 — checked with `git diff` over `Migrations/`, not assumed. ⚠ 1.21.0 DID mutate live data on boot (`EnsureBuiltInRolesAsync` creating the new role), so it was dumped first — **77,918,739 bytes / 104 CREATE TABLEs, verified as a dump rather than by a log line**. ⚠⚠ **AND THE ROLE WAS THEN CHECKED IN THE DATABASE, not by trusting the seeder**: "Add/edit stock" exists in both tenants with exactly `pos.items.manage` + `pos.stock.adjust` + `support.tickets`; `pos.items.manage` reached Owner/Company Admin/Store Manager/Supervisor and nothing else; **the Cashier holds neither — COUNT(*) = 0**. Rollbacks `~/PLUTUS/backend.pre-1.21.0` and `.pre-1.22.0`. ⚠ Both verified on swagger **200** AND `POST /api/v1/tokens/device` with a junk id → **401 "Device not enrolled or revoked."**, the axis that proves schema and model agree. **Previously (1.20.0):** | ✅ **DEPLOYED 2026-08-20 (late) & verified on the DB path** — swagger 200, and `POST /api/v1/tokens/device` with a junk (non-empty) GUID → **401 "Device not enrolled or revoked."**, which is the axis that proves the schema and the model agree. ⚠ **NO MIGRATION** — `ItemBarcodes` shipped with 1.19.0 and is unchanged; its six columns were re-verified live before the swap. Adds `PUT api/v1/items/{id}/barcodes/{code}` (atomic barcode correction), `GET api/v1/items/{id}/history`, and `{before, after}` audit writes on `ItemController.Put`. Rollback `~/PLUTUS/backend.pre-1.20.0`. ⚠⚠ **The endpoints were then EXERCISED LIVE**, not just probed: add → 201 · rename → 204 · a membership-card shape → 400 with its sentence · an interior space → 400 · a code another item owns → 409 naming it · a padded code → trimmed and accepted · history → 3 rows including the synthetic *"Created before change logging began"* bookend · **and an alias scan answered the CANONICAL `idOne`**, byte-identical to scanning the item's own barcode. Test aliases removed; `GET /api/v1/items/barcodes` is back to `[]`. |
| **Portal** | **1.17.0** (`index-C_yGEYNy.js`) | ✅ **DEPLOYED 2026-08-21 (afternoon)** — 512,638 bytes, verified on `admin.plutus.huggett.dscloud.me` (⚠ NOT `plutus.` — that is the till, and verifying a portal deploy against it tells you nothing). Carries **all 21 dialog ✕s** across 12 files, closing D4 on the third surface. `tsc --noEmit` clean; artefact checked for an unsubstituted Vite define **before and after** the copy to `current/`. Rollback `/srv/apps/PLUTUS/portal/current.pre-1.17.0`. ⚠ **Previously (1.16.0):** ✅ **DEPLOYED 2026-08-20 (late) & verified on all four axes** — right host (`admin.plutus.…`), served hash matches the build, **512,065 bytes** (not the ~1 KB SPA fallback), `1.16.0` and change-specific strings present, no unsubstituted defines, no `0.0.0`. Rollback `current.pre-1.16.0`. ⚠ The item editor gains the additional-barcode list (locked rows, atomic correction, live checks) and the change history. ⚠⚠ **AND ITS DIALOGS FINALLY GET A ✕** — the portal had **none, anywhere**, and D4 could not see it because its table listed only the two tills (§0.3e). |
| **Web till** | **1.31.0** (`index-DPOZBhUJ.js`) | ✅ **DEPLOYED 2026-08-21 (afternoon)** — 387,416 bytes on `plutus.huggett.dscloud.me`. Carries **Matt's receipt-printer fix** (§G72), the **login connection badge** (§G70) and **WP14's card hint** (§G69). ⚠⚠ **THE GATES RAN FOR THE FIRST TIME AT DEPLOY AND FOUND A REAL FAULT.** Final state `tsc` 0 · `eslint` 0 errors · **vitest 381/381 across 26 files** — but three of the new print tests went **RED** first, because `api.businessName()` read `localStorage` **unguarded** (its neighbour `getReceiptTemplateCached` has always been wrapped) and `printOnReceiptPrinter`'s catch turned the throw into a silent *"nothing printed"*. **Green would have been the wrong answer.** Guarded now — reading `localStorage` can THROW, not merely return null. Rollback `/srv/apps/PLUTUS/web/current.pre-1.31.0`. ⚠ **Previously (1.29.0):** ✅ **DEPLOYED 2026-08-20 (late) & verified on all four axes** — right host (`plutus.…`, **not** the portal's), served hash matches, **384,187 bytes**, `1.29.0` and change-specific strings present, no unsubstituted defines, no `0.0.0`. Rollback `current.pre-1.29.0`. ⚠ Deliberately the **same** barcode list and history as the portal's, under the 2026-08-19 look-and-feel ruling — an operator moving between the two must not find that only one of them can do it. |
| **MAUI till** | **1.113.0** (built, at `D:\tmp\plutus-till-1.113.0\`) | ✅ **BUILT 2026-08-21 (late)** and **launched for 25s with no crash and no crash log**. Carries **WP10** (an item's barcodes + history — §G74), §G69's MAUI half (the card-flow hint), §G73d (item editing refused for a cashier), and step 11b's orchestration fixes. ⚠ Version verified **in the assembly**: 3 refs to `1.113.0`, **zero** to 1.112.0 or 1.110.0 — MSBuild caches the evaluated version, so a bump can silently re-emit the previous one. ⚠ **THE ONLY BUILD ON THE BOX** — 1.110.0 deleted, so the folder listing is the truth. 169 MB / 268 root files / 88 subfolders, unchanged from the post-Syncfusion shape. ⚠ **Unpackaged**, so it shares no data with any installed MSIX: *"this till isn't enrolled"* on first run is expected. ⚠⚠ **25 SECONDS OF NOT CRASHING IS THE MOST AN AUTOMATED CHECK CAN SAY** — XAML and resource failures surface on **navigation**, and nothing in this repo covers a MAUI screen. **§G74 is the test that matters, and it needs a Supervisor AND a Cashier login.** ⚠ Sign out and in first: a token caches the permission set for 12h. **Previously (1.110.0):** ✅ **BUILT 2026-08-20 — SYNCFUSION IS OUT, and so is `DocumentFormat.OpenXml`.** **264 MB → 169 MB (−95 MB, −36%)** · 299 → 268 root files · 121 → 88 subfolders · **0** Syncfusion DLLs, **0** OpenXml. L4 CLOSED (see §10 L4) because its condition was met, not overridden. ⚠ The stale licence key is **deleted** from `App.xaml.cs`, so the trial-dialog hazard is structurally impossible rather than dormant. ⚠ `SatelliteResourceLanguages=en` removed all 33 satellite folders; the 86 native `.mui` ones stay and **cannot be moved** (see Shrink §6). ⚠ Verified: Release build 0 errors · MAUI suite **621** pass (was 625 — the 4 `ExcelHandlingTests` went with the class) · version stamped, no stale 1.109.0 string · **launched and ran 25 s**, log opening `Plutus MAUI till v1.110.0` with no licence or XAML failure. ⚠⚠ **HAND-RUN §G68 — a clean build and a 25-second launch are not a shift**, and the screens whose controls were swapped in the 1.33.0 work have no automated coverage. ⚠ 1.109.0 deleted, so the folder listing is unambiguous. |
...
## 7. How long, honestly — **≈5–7 days** (re-costed 2026-08-22)
...
>
> ⚠ **So: not finished, but nothing like a two-month job.** ~~Roughly **10–15 days**~~ → ~~≈7–9 days~~ → **≈5–7 days** (Step 11b closed 2026-08-22)
