...
| Web till | **1.20.0** (`index-iuWpo9-8.js`) | ✅ **DEPLOYED 2026-08-19 (evening) & verified on all four axes** — right host (`plutus.…`, verified there and not on the portal's), the served HTML names the new hash, **369,708 bytes** (the real bundle, not the ~1 KB SPA fallback), `1.20.0` substituted, and `reports/published` present in the SERVED bundle. Rollback `current.pre-1.20.0` = 1.19.0. ⚠ Carries the D4 dialog fixes (✕ + Escape on Ask/HelpPanel/Receipt) and 5b(a)'s web half. |
