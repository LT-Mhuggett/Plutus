...
| till-maui | **1.81.0** | ✅ **BUILT 2026-08-18** at `D:` `\tmp\plutus-till-1.79.0`, artefact reads `1.81.0+a5d571c6` (= HEAD, so the artefact provably contains every commit); every change verified present in the binary by string check, not by commit. Carries Store Information (1.74.0), the Reports fix (1.75.0), the refund crash + adjust exit (1.76.0), the ✕ on every dialog (1.77.0), the three crashes — Save Transaction / adjust ✕ / overlapping loyalty buttons (1.78.0) — and the **price-adjust VAT fix** (1.79.0). ⚠ `plutus-till-1.79.0` is also on disk — it was RUNNING when the delete was attempted; everything older is gone. ⚠ Still **🟡 — nobody has run it** |
| Backend | **1.17.2** | ✅ **DEPLOYED 2026-08-18 & verified** on the DB path (`POST /api/v1/tokens/device` answers, not `/swagger`), report routes reached (401 unauthenticated rather than 404/500), no startup errors. Rollback `~/PLUTUS/backend.pre-1.17.2`. ⚠ Carries the **`pos.reports.view` alternative on the five report endpoints that lacked it** — see B-reporting |
...
| 3b | ⚠⚠ **A MONEY BUG FOUND BESIDE IT: a second scan joined a hand-adjusted line AT THE ADJUSTED PRICE** | ✅ **FIXED 2026-08-18 (till 1.86.0).** Ring a £5 item, adjust it to 50p, **leave the line selected**, scan the same item again — and the second unit joined that line at **50p**. The shop sold it for a tenth of its price, silently, with the receipt as the only evidence. ⚠ **The fault was TWO PATHS, not a missing check.** MAUI's search path compared the price pair (and so behaved correctly by arithmetic accident); its *selected-line fast path* checked only the item id and "not a return". The web till has never done this — `basket.ts` excludes `!l.adjusted`. ⚠ Found by **reading the web till's reducer instead of ours** while doing item 3, which is the lesson: the recorded rule is *when Matt says a screen does not match, check the other screen before defending ours*, and it pays on logic as well as layout. ✅ The rule now lives in `SharedKernel.BasketMerge` (**C1**), both MAUI call sites ask it, and its TypeScript twin is pinned by a **shared ten-case vector table** (**C2**) — `BasketMergeTests` and `till/basketMerge.test.ts`, same figures. ⚠ Two mutants watched dying: removing the `adjusted` guard, and dropping the ex-half of the price comparison. ⚠⚠ **The `adjusted` flag is tested BEFORE the price pair** — adjusting a line to exactly its catalogue figure would otherwise pass the arithmetic and re-merge, which is the accident the old code relied on. ⚠ Honest weakness recorded in C2: the TS side is a **copy under test**, because the web till's predicate lives inside a `find(…)` inside `addLine` and is not exported. **Exporting it is the real fix and is not done.** ⚠ **§G42d** is the hand-check, and it is written as a money check | **found and fixed** |
...
| 6 | **Loyalty on MAUI is unusable** | 🟡 **HALF DONE 2026-08-18 (till 1.82.0)** — see 6b. **Add member** and **Set tier** now exist on the Loyalty tab, the mandatory field is marked, and every failure is said out loud. ⚠ **What remains: EDIT, the portal's information, and the layout.** Edit is the awkward one and needs a ruling, not just code — `PUT /api/v1/customers` can change an email, and an email change silently redirects somebody's account, so the till has deliberately never had an edit path. Either it is restricted to name/phone (safe, and probably what a shop wants) or it needs the same audit shape as a discount. The list itself shows Member / Tier / Credit; the portal also shows renewal date and lifetime spend | **~1 d left** of 1½–2 d |
