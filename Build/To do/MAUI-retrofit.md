...
| 6 | **Loyalty on MAUI is unusable** | 🟡 **HALF DONE 2026-08-18 (till 1.82.0)** — see 6b. **Add member** and **Set tier** now exist on the Loyalty tab, the mandatory field is marked, and every failure is said out loud. ⚠⚠ **THE EDIT RULING CAME 2026-08-18 and edit is ALLOWED** — Matt: *"A customer needs to have a unique ID, because people can change emails over time. Audit please."* The unique id already exists (`Customer.Id`, a UUIDv7; `MemberNo` is the human-facing one) and **nothing resolves a customer by email**, so changing one cannot redirect an account — which removes the objection this row was built around. ✅ **The audit half is DONE (backend 1.17.4)**: `PUT /customers/{id}` recorded only the request body, i.e. what the customer BECAME and never what they WERE, so an audit could not show that an email had changed at all. It now records `before` and `after`, pinned by a test whose mutant was watched dying. ⚠ **What remains is the till SCREEN** — an edit dialog on the Loyalty tab, plus the portal's information and the layout — `PUT /api/v1/customers` can change an email, and an email change silently redirects somebody's account, so the till has deliberately never had an edit path. Either it is restricted to name/phone (safe, and probably what a shop wants) or it needs the same audit shape as a discount. The list itself shows Member / Tier / Credit; the portal also shows renewal date and lifetime spend | **~1 d left** of 1½–2 d |
