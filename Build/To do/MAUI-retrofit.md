...
| till-maui | **1.79.0** | ✅ **BUILT 2026-08-18** at `D:` `\tmp\plutus-till-1.79.0`, artefact reads `1.79.0+8ff8900b`; every change verified present in the binary by string check, not by commit. Carries Store Information (1.74.0), the Reports fix (1.75.0), the refund crash + adjust exit (1.76.0), the ✕ on every dialog (1.77.0), the three crashes — Save Transaction / adjust ✕ / overlapping loyalty buttons (1.78.0) — and the **price-adjust VAT fix** (1.79.0). ⚠ It is the ONLY build on the box; every earlier one is deleted. ⚠ Still **🟡 — nobody has run it** |
