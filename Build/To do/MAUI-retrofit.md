...
| till-maui | **1.75.0** | ✅ **BUILT 2026-08-18** at `D:\tmp\plutus-till-1.75.0`, artefact reads `1.75.0+d6a993bd`. Adds the rebuilt **Store Information** screen (§G33/W9d) **and the Reports-tab fix** — both verified present in the binary by string check, not just by commit. ⚠ **1.74.0 is still on disk**: it was RUNNING when the delete was attempted (file lock), so for once there are two builds. Close it and remove `D:\tmp\plutus-till-1.74.0`. ⚠ Still **🟡 — nobody has run it** |
