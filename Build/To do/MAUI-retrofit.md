...
| till-maui | **1.74.0** | ✅ **BUILT 2026-08-18** at `D:\tmp\plutus-till-1.74.0`, artefact reads `1.74.0+1e456511` = HEAD. Adds the rebuilt **Store Information** screen (§G33/W9d) — verified present in the binary by string check, not just by commit. **1.73.0 deleted**, so there is exactly one build on the box. ⚠ Still **🟡 — nobody has run it** |
