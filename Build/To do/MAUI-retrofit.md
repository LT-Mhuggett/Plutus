...
| till-maui | **1.99.0** | ✅ **BUILT 2026-08-19** at `D:\tmp\plutus-till-1.99.0`; `1.99.0+2ac156a9` = HEAD. Verified inside the binary: all seven strings from 1.96.0–1.99.0, the shared `ReportCatalogue`, the till's `PublishedReports`, and the mojibake form confirmed **ABSENT**. ⚠ The only build on the box; 1.94.0–1.97.0 deleted. ⚠⚠ Carries **four live defects fixed** — the tender cap (§G50), money moving before the sale is recordable (§G53), the garbled customer-facing text (§G52), and the two ungated checkout settings (§G55b) — plus 5b(a)'s till half and 5b(b). ⚠ Runs against live backend **1.17.8** for everything except §G54, which needs 1.17.10 + portal 1.11.0. |
| Web till | **1.20.0** (`index-iuWpo9-8.js`, 369,708 bytes) | 🟡 **BUILT 2026-08-19, NOT DEPLOYED.** In `~/PLUTUS/Plutus.Frontend.WebApp/dist` on the Mac; version confirmed IN the bundle. Carries the D4 dialog fixes (✕ + Escape on Ask/HelpPanel/Receipt) and 5b(a)'s web half. **Deploy:** take `current.pre-1.20.0`, then `rsync -a --delete dist/ /srv/apps/PLUTUS/web/current/`. ⚠ Verify against `plutus.…`, NOT `admin.plutus.…`. |
| Portal | **1.11.0** (`index-PCgKPhQv.js`) | 🟡 **BUILT 2026-08-19, NOT DEPLOYED.** In `~/PLUTUS/Plutus.Frontend.Portal/dist`; version AND "Reports on the tills" both confirmed in the bundle. Carries 5b(a)'s curation screen. **Deploy:** take `current.pre-1.11.0`, then `rsync -a --delete dist/ /srv/apps/PLUTUS/portal/current/`. ⚠ Verify against `admin.plutus.…`. |
| Backend | **1.17.10** | 🟡 **COMMITTED, NOT DEPLOYED — live is 1.17.8** (`GET /api/v1/ping`). Carries 5b(b)'s endpoint re-gate and 5b(a)'s controller + storage. ⚠⚠ **IT CARRIES THE `AddReportPublications` MIGRATION**, and migrations auto-apply on startup — **dump the database first, and check the dump is not 20 bytes** (2026-08-11: every nightly backup was 20 bytes for two days while the log said `backup ok`). ⚠ Nothing already deployed needs it: the re-gate only WIDENS report gates, and a till with no answer from `reports/published` shows every report. |
