...
| till-maui | **1.76.0** | ✅ **BUILT 2026-08-18** at `D:	mpplutus-till-1.76.0`, artefact reads `1.76.0+ac1d41b1`; the fixes verified present in the binary by string check. Carries Store Information (1.74.0), the Reports-tab fix (1.75.0) and now the **refund crash** + **adjust-box Cancel** + the cancel-crash that fix uncovered. ⚠ `plutus-till-1.75.0` is still on disk — it was RUNNING when the delete was attempted; 1.73.0/1.74.0 are gone. ⚠ Still **🟡 — nobody has run it** |
