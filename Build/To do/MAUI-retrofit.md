...
| Web till | **1.18.0** (`index-CEE3U63P.js`, css `index-iaJiIr0W.css`) | ✅ **DEPLOYED 2026-08-18 (late) & verified on all four axes** — right host (`plutus.…`, not the portal's `admin.plutus.…`), served hash matches the build, **367,374 bytes** (not the ~1 KB SPA fallback), and "Print card" present in the SERVED bundle. Rollback `current.pre-1.18.0` = 1.17.0. ⚠ Carries **WP-L1b's web half**: Open on every loyalty row → the customer's facts, their whole history in a `DataTable`, Grant credit, and a **CR80 printable card**. Portal confirmed untouched. |
| till-maui | **1.94.0** | ✅ **BUILT 2026-08-18 (late)** at `D:\tmp\plutus-till-1.94.0`; `1.94.0+6eae51d3` = HEAD, four strings verified inside the binary. ⚠ The only build on the box. ⚠ Needs backend 1.17.8. ⚠⚠ Carries **§5c items 8 + 9**: the bag item moved to **Settings → Till**, the section names are the web till's, and **the "Plutus" tab is GONE** — its screen is now Settings → Till device. |
