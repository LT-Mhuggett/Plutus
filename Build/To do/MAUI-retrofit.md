...
| till-maui | **1.77.0** | ✅ **BUILT 2026-08-18**, artefact reads `1.77.0+31a485f6`; every change verified present in the binary by string check, not by commit. Carries Store Information (1.74.0), the Reports-tab fix (1.75.0), the refund crash + adjust-box exit (1.76.0) and the **✕ on every dialog** (1.77.0, till-design **D4**). ⚠ Folder path: `D:` `\tmp\plutus-till-1.77.0`. ⚠ `plutus-till-1.75.0` is still on disk — it was RUNNING when the delete was attempted; 1.73.0/1.74.0/1.76.0 are gone. ⚠ Still **🟡 — nobody has run it** |
