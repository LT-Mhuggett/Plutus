...
| **12** | **Server-side refund cap = quarantine (202), not rejection.** `SalesIngestService` calls `SharedKernel.RefundRules.Authorise` against the origin sale's recorded refunds; an over-refund quarantines with a reason, exactly like the VAT quarantine path. Never a 400 — quarantine is visible and reviewable, and never refuses at a counter. ⚠ **The one default Matt is most likely to want to veto — confirm before Step 17.** | The cap is client-only on both tills today; a modified till can over-refund silently. Quarantine survives a compromised client without ever eating a legitimate sale. | Step 17 |
| **13** | **Tenders = the fixed `TenderType` set (Cash, Card, Online, Credit, GiftCard), moved to SharedKernel.** No payment-method roster endpoint; the legacy `/api/PaymentMethod/Index` is not consumed. The web till uses the same closed set (`api.ts tenderTypeFor`). | A roster endpoint for five constants is machinery without a requirement. | Steps 8, 9 |
