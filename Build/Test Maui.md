...
| | |
|---|---|
| **Run** | ✅ `D:\tmp\plutus-till-1.99.0\Plutus.Frontend.AppClient.exe` — double-click, nothing to install. **BUILT 2026-08-19 and verified inside the binary** (`1.100.0+86e91eea` = HEAD; the trap fix, printer fix and theming fixes verified inside the binary.96.0–1.99.0 confirmed present, the shared `ReportCatalogue` and the till's `PublishedReports` present, and the mojibake form confirmed **absent**. ⚠ **The only build on the box** — 1.94.0–1.99.0 deleted. ⚠ Treat the FOLDER LISTING as the truth: the version written in this prose has been wrong twice in one day. ⚠⚠ **START HERE, IN THIS ORDER — the four live defects first:** **§G52** (30 s, the garbled text), **§G50a** (2 min, store credit spent twice), **§G53a** (Z-close the day then ring a store-credit basket — and **check the member's balance afterwards**), **§G55b** (a cashier can no longer change the checkout options). Then §G38–§G49 and §G51/§G54. ⚠ **EVERYTHING IN THE LIST IS NOW TESTABLE** — backend **1.17.10**, portal **1.11.0** and web till **1.20.0** all deployed and verified 2026-08-19 (evening), so §G54 (report curation) works too. ⚠ A permission change needs a **sign-out/in**: login tokens cache for 12h with the effective permission set baked in (pitfall 10), which matters for §G51 and §G55b. ⚠⚠ **THE "PLUTUS" TAB IS GONE** — it is now **Settings → Till device**. ⚠ It will say the till isn't enrolled; expected for an unpackaged build. |
