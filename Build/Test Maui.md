...
> ⚠⚠ **`MAUI_finaltest.md` WAS MERGED INTO THIS FILE AND DELETED, 2026-08-25.** Matt: *"Can you merge the
> MAUI_Finaltest.md and Test_Maui.md documents please. No reason having 2 docs."* Right: one said
> *what is unproven and why*, the other said *what to click*, and keeping them apart meant the status
> at the top of one was always a day behind the script in the other. **This is now the only live MAUI
> document.** Its old section numbers are kept — see *Why the numbers are not sequential* below.
>
> The retrofit's history is [`archive/MAUI-retrofit.md`](archive/MAUI-retrofit.md); the rulings and
> binding defaults are [`till-design.md`](till-design.md) **Part E**.

Anyone can run this. You do not need to know the codebase, and you should not need to ask anyone what
a step means — if a step is unclear, that is a bug in this document, so please say so.

**Time:** about **3 hours** for everything. About **15 minutes** for the "start here" list below,
which is the part worth doing if that is all the time you have.
...
## ⚠⚠ What build you are testing — check this first

| | |
|---|---|
| **MAUI source version** | **1.121.0** |
| **Newest artefact anywhere** | **1.120.0**, at `D:\tmp\plutus-till-1.120.0\` |
| Web till (live) | **1.37.0** |
| Portal (live) | **1.26.0** |
| Backend (live) | **1.33.0** |

⚠⚠ **SO THE SOURCE IS AHEAD OF EVERY ARTEFACT, AND THAT HAS ALREADY WASTED A TEST RUN.** On
2026-08-20 Matt reported the basket buttons broken for the second time and the fix was sitting in an
undeployed bundle. **A fix that is built and not shipped is indistinguishable from a fix that was
never made** — so before running anything, check the version in the app against the row above, and if
a step tests something newer than the artefact you have, say so rather than failing it.

⚠ MAUI builds **only when Matt asks** (his ruling, 2026-08-16). If a section needs a newer till,
that is a build request, not a test failure.

## ⚠⚠ Where this actually stands, and what "done" means

**The build is finished. Nothing here is a code estimate.** Re-derived from
[`till-design.md`](till-design.md) A0 with `awk` rather than counted by hand (2026-08-25):
...
The only ⬜s left anywhere in Part B are not MAUI build work: **Store Information's local editor** is
a deliberate ⬜ because cutover step 20 *deleted* it (that row is inverted — a removal, not a build),
and **remote lock of a lost till** is ⬜ on the **web till too**, moved out to platform WP-SL.

⚠⚠ **SO THE ENTIRE REMAINING RISK IS 41 🟡 MAUI ROWS (AND 21 ON THE WEB TILL): built, tested where
testable, and never once exercised by a person.** A 🟡 is not a smaller ⬜ — it is an unknown. Only
somebody at a screen turns one into a ✅.
...
see**, and the record is at the bottom of this document:
...
Real gaps a ✅/✅ parity row cannot express. They come from §5c below.
...
## ⚠ Why the numbers are not sequential

The section numbers in the reference half of this document are the numbers they had in
`MAUI-retrofit.md` and the old `MAUI_finaltest.md`, kept **deliberately**. `till-design.md` and the commit
history cite them by number — *"§5c item 10"*, *"§0.3b"*, *"see §8"* — and renumbering would
silently break every one of those references while looking tidier. A gap in the numbering costs a
moment's confusion; a reference that resolves to the wrong thing costs a session.

⚠ **What is MISSING from a till is not in this document.** Platform-wide gaps, open decisions and
known holes live in [`Platform Gaps.md`](Platform%20Gaps.md); per-capability state is
[`till-design.md`](till-design.md) A0/Part B. This page is *how to prove the build works*.
...
> ⚠ **Generated from the headings.** If a row here is wrong, the heading is wrong — fix that.
> ⚠⚠ **NO STATUS COLUMN, DELIBERATELY.** Which tests to run *now* lives in **START HERE** above,
> in one place. A second list of states is a second thing to forget, and stale status is the
> complaint this contents list was added to answer (Matt, 2026-08-21: *"a lot of these tests are
> stale"*).

**Sections:** Test Maui · §A · §B · §C · §D · When you are done · §E · F. Discounts · G. Members and the tier discount · §W

| # | Test | # | Test |
|---|---|---|---|
| **G1** | FIRST — does the bottom of the till screen still look right? | **G39** | Screens must update while you look at them |
| **G2** | Attach a member by searching | **G40** | Stock and Negative stock |
| **G3** | The discount actually appears, and it is right | **G41** | Drill-down — open a sale from a report. Till 1.85.0, §5c item 5 |
| **G4** | Scan a membership card | **G42** | Tap the price to adjust it — and a money bug found next to it. Till 1.… |
| **G5** | The exclusions — this is the money-correctness step | **G43** | The basket rows are styled and themed at last |
| **G6** | Remove the member | **G44** | The Loyalty tab is the web till's screen now |
| **G7** | An expired membership | **G45** | The Loyalty table's layout |
| **G8** | Sell it, and check the receipt and the record | **G46** | Colours — the whole app, and a DARK scheme that looks dark. Till 1.91.… |
| **G9** | Add a new member at the till | **G47** | The customer detail view — the portal's dialog on a till. Till 1.92.0,… |
| **G10** | Set a member's tier — Supervisor and up | **G48** | Print card — and it is a DIFFERENT object on each till. Till 1.93.0 + … |
| **G11** | Store credit as a payment | **G49** | Settings, and the tab that moved into it |
| **G12** | The store-credit refusals — these are the money ones | **G50** | A tender cannot take its cap twice — the money defect of 2026-08-19. T… |
| **G13** | Pay with a gift card | **G51** | Per-report permissions — a narrow role. Backend 1.17.9 + till 1.96.0 +… |
| **G14** | The gift-card refusals | **G52** | The text a customer can read. Till 1.97.0 — 30 seconds, do it first |
| **G15** | Sell a gift card | **G53** | Money must not move when the sale cannot be recorded. Till 1.98.0 |
| **G16** | Gift-card VAT — the one that cannot be seen on screen | **G54** | The portal decides which reports a till shows. Portal 1.11.0 + backend 1… |
| **G17** | A member discount must not touch a gift card | **G55** | Settings works like the web till's, not like a list of buttons. Till 1.9… |
| **G18** | The Loyalty tab | **G56** | The four faults from Matt's 1.99.0 hand-run |
| **G19** | The table controls | **G57** | Carrier bags come from the PORTAL now |
| **G20** | The Reports tab | **G58** | THE CHECKOUT IS ONE SCREEN NOW |
| **G21** | Reports vs the web till — the figures must MATCH | **G59** | Typing a member number attaches the member |
| **G22** | Reprint a receipt from ANOTHER till | **G60** | The theme reaches the whole app |
| **G23** | The receipt now obeys the PORTAL | **G61** | The portal's Loyalty page |
| **G24** | MONEY ON EVERY BASKET ROW — the most important check in 1.69.0 | **G62** | A long basket never hides the buttons |
| **G25** | The noticeboard — pick notes and platform announcements | **G63** | The Company page collapses like Locations |
| **G26** | Help and support — ask Plutus, and read the reply | **G64** | Scheduled discounts at the counter |
...
**§1b of this document** (see *The record*, at the bottom) and worked through from there. (Older runs
live in `archive/handrun-2026-08-11.md`.)
...
⚠ **CORRECTED 2026-08-21: there is a Reports tab and the Statistics tab is GONE from the tab bar** (hidden 2026-08-16, not deleted — L4). The note below was written while both were visible. It is deliberate for
...
counter. On the Settings tab it would be a notice nobody reads.
...
1. **Settings → Till device** → **Ask for this till to be removed** → confirm. ⚠ *(Was "Plutus tab"; that tab was folded into Settings on 2026-08-18.)*
...
4. **Reject** the removal in the portal. Within a minute **Settings → Till device** shows the till Active again.

### G27b. ⚠⚠ Now actually revoke it

1. In the portal, **approve** the removal (or revoke the device directly).
2. **Leave the till alone and watch it — do not touch Settings → Till device.**
...
2. Reprint a past receipt (**Reports tab → Reprint a receipt**). ⚠⚠ *(Was "Plutus tab → reprint". It was on the STATISTICS tab, which came off the tab bar on 2026-08-16 — so reprint was unreachable on MAUI until it was moved to Reports on 2026-08-21.)*
...
⚠⚠ **REPORT ANY CRASH HERE IMMEDIATELY — AND IT WOULD NOW BE A NEW FAULT.**

⚠ **CORRECTED 2026-08-25.** This step used to say *"17 other call sites still mishandle a cancelled
dialog"*. **That was never true and it was closed on 2026-08-21.** Re-enumerated by grep: **23**
`LaunchInputAlertAsync` call sites, and **every one an operator can open** returns safely on
`Count == 0` or a checked `TryGetValue` — including all five that used to be named as risky here
(refund, selling a gift card, cash, the supervisor prompt, checkout). D4's dialog contract is **5/5
honoured**; see §0.3b below for the audit and what the real residue turned out to be.

**So if a dialog dies when you cancel it, that is a NEW fault, not a known one.** Say which dialog
and what you were cancelling — do not assume it is expected.
...
⚠ **CORRECTED 2026-08-25:** this used to warn that *"17 call sites still mishandle a back-out"* — that
number was never right and the audit closed on 2026-08-21 (23 call sites, every reachable one returns
safely; D4 is **5/5**, see §0.3b). **A crash on back-out is therefore a NEW fault** — tell me which
dialog and what you cancelled.
...
**Reports tab.** Ring another sale, then go to **Reports**, and stay there. ⚠ *(Said "Statistics" until 2026-08-21; that tab has not been on the tab bar since 2026-08-16.)*
...
> and the web till never had one. Its contents are now **Settings → Till device**. The tabs today are
> **Till · Cash · Inventory · Reports · Loyalty · Store Options · Settings**.
...
### G68b. Reports — the screen that changed most

**Reports tab.**

> ⚠⚠ **THIS STEP SAID "STATISTICS TAB" UNTIL 2026-08-21, AND THERE HAS NOT BEEN ONE SINCE
> 2026-08-16.** Reports replaced it (step 26) because the old tab's three viewmodels read the LEGACY
> LOCAL DATABASE, which has had nothing written to it since cutover step 11 — so it showed £0.00 for
> everything sold since. **The checks below were always about the Reports screen**; only the name was
> wrong. ⚠ Statistics is hidden rather than deleted, on Matt's instruction, and removing it is L4.
...
⚠ **If anything crashes, grab the crash log before restarting** — **Settings → Till device** shows its path.
...
1. Ring up a sale **on the MAUI till** and let it reach the server (**Settings → Till device** → queue empty).
...
# Reference — what only a person can close, and what has already been found
