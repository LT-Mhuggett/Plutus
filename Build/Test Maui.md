...
| | |
|---|---|
| **Run** | ✅ `D:\tmp\plutus-till-1.99.0\Plutus.Frontend.AppClient.exe` — double-click, nothing to install. **BUILT 2026-08-19 and verified inside the binary** (`1.99.0+2ac156a9` = HEAD): all seven strings from 1.96.0–1.99.0 confirmed present, the shared `ReportCatalogue` and the till's `PublishedReports` present, and the mojibake form confirmed **absent**. ⚠ **The only build on the box** — 1.94.0–1.97.0 deleted. ⚠ Treat the FOLDER LISTING as the truth: the version written in this prose has been wrong twice in one day. ⚠⚠ **START HERE, IN THIS ORDER — the four live defects first:** **§G52** (30 s, the garbled text), **§G50a** (2 min, store credit spent twice), **§G53a** (Z-close the day then ring a store-credit basket — and **check the member's balance afterwards**), **§G55b** (a cashier can no longer change the checkout options). Then §G38–§G49 and §G51/§G54. ⚠⚠ **NEEDS BACKEND 1.17.10 AND PORTAL 1.11.0 FOR §G54** — both are BUILT but **NOT DEPLOYED**; live backend is **1.17.8**. Everything else in the list is testable against 1.17.8. ⚠ The backend deploy carries the `AddReportPublications` **migration**, which auto-applies on startup — dump the database first **and check the dump is not 20 bytes**. ⚠⚠ **THE "PLUTUS" TAB IS GONE** — it is now **Settings → Till device**. ⚠ It will say the till isn't enrolled; expected for an unpackaged build. |
