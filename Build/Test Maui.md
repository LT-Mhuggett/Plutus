...
| | |
|---|---|
| **Run** | ✅ `D:\tmp\plutus-till-1.100.0\Plutus.Frontend.AppClient.exe` — double-click, nothing to install. **BUILT 2026-08-19 (evening) and verified inside the binary** (`1.100.0+86e91eea` = HEAD): the Till-device Close handler, both printer sentences, the theming resource keys, and 1.98.0's money-fix wording all confirmed present. ⚠ **The only build on the box** — 1.94.0–1.99.0 deleted. ⚠ Treat the FOLDER LISTING as the truth: the version written in this prose has been wrong three times in one day, including by the very edit that added the warning. ⚠⚠ **START WITH §G56** (the four faults Matt found on 1.99.0 — the Till-device trap, the printer value, the white pills, the invisible buttons/switches/✕), then the four live defects if not yet run: **§G52, §G50a, §G53a, §G55b**, then the rest. ⚠ **EVERYTHING IS TESTABLE** — backend **1.17.10**, portal **1.11.0**, web till **1.20.0** all deployed and verified. ⚠ A permission change needs a **sign-out/in** (12h token cache) — matters for §G51/§G55b. ⚠⚠ **THE "PLUTUS" TAB IS GONE** — Settings → Till device, and it now has a **Close** (§G56a). ⚠ It will say the till isn't enrolled; expected for an unpackaged build. |
