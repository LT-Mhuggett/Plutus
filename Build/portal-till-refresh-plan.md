...
| **P1** WP1.3 Migrate remaining legacy tables (sweep) | 🟡 partial | Portal **Customers** + **Loyalty** and till **Loyalty** now use the standard DataTable (client mode ≤500 rows — no backend/shape change, avoids touching the shared `/customers` endpoint the till's `searchCustomers` depends on). Still hand-rolled (low value / tiny / high-churn — left): Stores sub-tables, VAT band table, ItemsSold, till InventoryPage list. **LIVE 2026-07-30** (`095eb21`; rollbacks portal/till `current.pre-p13`). |
...
| **P6** WP6.1–6.3 Store info / un-enrol approval / Help | ✅ LIVE | **6.1** till Store Information now READ-ONLY from new `GET /api/v1/stores/{id}/info` (sales.ingest-gated); **6.2** un-enrol-with-approval — `DeviceStatus.PendingRemoval` (a byte enum value → **NO migration**, plan's assumption was wrong), token issuance still allowed while pending (keeps trading), `POST /tills/unenrol-request` + `/tills/devices/{id}/removal` + `/devices/{id}/status`; till Settings gated on portal.tills.enrol + confirm + poll-until-revoked, portal Locations till card shows Pending removal + Approve/Reject; **6.3** two new perms `support.tickets` (seeded to EVERY role) + `pos.settings.manage` (Owner/CompanyAdmin/StoreManager), support endpoints gated on support.tickets, till Help moved to the appbar (❓) with threaded history + reply, Settings device-toggles gated on pos.settings.manage. Tests: `TillHardeningE2eTests` (un-enrol flow + gating + seeding). Suites Unit 214 / Arch 6 / Integration 49. **LIVE 2026-07-30** (`4d422a9`): backend+portal+till swap + **`Plutus.SeedMigrator rbac`** run on the Mac to backfill the two perms (verified: support.tickets on 8 roles, pos.settings.manage on 3). NO migration. Rollbacks `backend.pre-p6`, portal/till `current.pre-p6`. |
...
**Resume point:** **✅ PLAN COMPLETE — P1–P6 all DONE and LIVE (2026-07-30).** Only optional leftover: the P1.3 legacy-table DataTable sweep (CustomersPage/VatPage/StoresPage sub-tables etc.). New work would be a fresh plan.
