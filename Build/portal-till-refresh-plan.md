...
| **P6** WP6.1–6.3 Store info / un-enrol approval / Help | 🟢 committed | **6.1** till Store Information now READ-ONLY from new `GET /api/v1/stores/{id}/info` (sales.ingest-gated); **6.2** un-enrol-with-approval — `DeviceStatus.PendingRemoval` (a byte enum value → **NO migration**, plan's assumption was wrong), token issuance still allowed while pending (keeps trading), `POST /tills/unenrol-request` + `/tills/devices/{id}/removal` + `/devices/{id}/status`; till Settings gated on portal.tills.enrol + confirm + poll-until-revoked, portal Locations till card shows Pending removal + Approve/Reject; **6.3** two new perms `support.tickets` (seeded to EVERY role) + `pos.settings.manage` (Owner/CompanyAdmin/StoreManager), support endpoints gated on support.tickets, till Help moved to the appbar (❓) with threaded history + reply, Settings device-toggles gated on pos.settings.manage. Tests: `TillHardeningE2eTests` (un-enrol flow + gating + seeding). Suites Unit 214 / Arch 6 / Integration 49. NOT deployed (RBAC re-seeds on startup; no migration). |
