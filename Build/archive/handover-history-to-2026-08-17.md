# Handover history — 2026-07-28 to 2026-08-17

> ⚠ **ARCHIVED 2026-08-17.** Matt: *"I only want the handover to be for the following day."*
> [`HANDOVER.md`](../../HANDOVER.md) is now a **next-session brief** and nothing else. Everything that
> is still **live** — rulings, open items, deploy state, unactioned findings — moved into
> [`Build/To do/MAUI-retrofit.md`](../To%20do/MAUI-retrofit.md), which is the one plan document.
>
> **This file is the day-by-day narrative, kept verbatim and dropped from nothing.** It is history, not
> instruction: every "START HERE", "TOMORROW, IN ORDER" and "RESUME HERE" below is **superseded**, and
> the version numbers, deploy states and open-fault lists were true on their date and are not now.
>
> ⚠ **Read it for the REASONING, never for the state.** It is the record of how fourteen faults were
> found on 2026-08-10 (six invisible to every automated test), how the checkout deadlocked on
> 2026-08-13, and why several "done" markers were wrong — the kind of thing that stops a mistake being
> made twice. For what is true today, start at `HANDOVER.md`.
...
**3b. ⚠⚠ THEN A4 (OVERPAY BY CARD) CRASHED 1.49.0 — FIXED IN 1.49.1, AND IT HAD BEEN HIDING BEHIND THE DEADLOCK.**
...
**3c. THEN 1.49.2 — a split payment worked and looked broken.** Matt: *"an item is £4.40, I press cash,
put in £2, it takes me back to the 'Card or cash' screen but doesn't tell me anything has been paid or
there is X to pay. I assume its not actually working."* ✅ **It was working** — the accumulation has been
pinned since the tender loop was extracted — but the tender picker's title read *"Payment Method"* on
the second pass exactly as on the first, and the balance only appeared one screen later.

⚠ **"Works and looks broken" is not a smaller problem than "broken", it is a worse one:** an operator
who cannot tell a working split payment from a swallowed £2 will stop splitting payments, or take the
money twice with a customer watching. Now `Paid £2.00 — £2.40 left to pay`.

⚠ **The figure comes from `TenderLoop`, not the basket**, because the loop adds the card surcharge to
what is owed — a caller deriving `paid = myTotal − outstanding` would be right until a tenant switched a
card fee on, and then wrong **by the fee**, on a screen telling an operator how much money they hold.
`chooseMethod` takes `(outstanding, paidSoFar)` and a test pins the surcharge case specifically.
**Unit 907 → 909.** ⚠ **`platform` bumped to 1.27.0 as well** — a `src/` library changed, and I nearly
shipped without it.
...
**3l. ⚖ DISCOUNTS — Matt's three rulings, binding default 22. One built, two blocked on findings.**
*(Matt, 2026-08-13: "You cannot have a discount greater than the basket. Discount levels should be a
setting configurable by the owner and over certain levels (which can be added and configurable) need
approval from a supervisor or higher. All discounts need to be tracked — till, logged-in employee and
reason.")*

✅ **(a) BUILT — `SharedKernel/BasketDiscounts.cs`.** Same shape as `RefundRules`: a verdict plus
amounts, with **no money baked into a string** — ⚠ I had written `£` into the messages, and
`RefundDecision`'s own comment caught it (formatting is a client concern, wrong the first time a
tenant trades in another currency, and unlocalisable for MAUI's `I18N_L10N`). Headroom is **net of what
is already off**, because the commit-time apportioner judges it that way and a gate that disagrees
passes a basket through one and throws at the other. ⚠ Boundary **inclusive** — a 100% staff discount
is legitimate; only *more* than everything is refused. Returns are not headroom. 15 tests,
mutation-checked (exclusive boundary, ignoring prior discounts, and negative headroom all caught).
⚠ **Not yet wired into `TillViewModel`'s apply path**, so the un-completable-basket defect is still
live; when it is wired the `DEFECT_…` test fails by design and must be rewritten.
...
2. ⚠⚠ **The MAUI percentage path does not use the shared rule.** `TillViewModel` (~1030, ~1046)
   ...
   multiplied the price BY TEN"*), with the shared fix never called from here. ⚠ Confirm what units
...
3. ⚠⚠ **The wire carries no reason and no actor for a discount**, so (c) is impossible today.
   `LineDiscount` is `{ id, rate }`. The sale records who *rang* it and the till, never who
   *authorised* a discount or why. Precedent next door: `SaleAdjustmentDto` has a `Reason` for refunds
   and voids. Additive change (`reason` + `authorisedBy`), and the authoriser is **the whole point of
...
# ⚠ 2026-08-21 handover — archived verbatim on 2026-08-25

> Archived when Matt asked for HANDOVER.md to be emptied between sessions: *"Handover should be
> empty as we are working, then a new one created for tomorrow when I finish tonight."* Its durable
> content went to `Platform Gaps.md` (what is missing), `repo-runbook.md` (how things work),
> `Test Maui.md` (what to prove) and `till-design.md` Part E (the rulings). Kept here because the
> narrative explains WHY several of those entries exist.
>
> ⚠ **This file's name says "to 2026-08-17" and it now carries later entries too.**
