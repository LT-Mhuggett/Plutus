...
> ## 📦 ARCHIVED 2026-08-23 — the retrofit is BUILT. Nothing here is outstanding work.
>
> **Matt:** *"If MAUI-Retrofit.md is finished, please strip out all of the tests into a
> MAUI_finaltest.md doc and archive MAUI-retrofit.md."*
>
> It was finished. Every cutover step, every work package and all sixteen L rows are closed, and
> §7's own table — re-derived from `till-design.md` with `awk`, not counted by hand — put the A0
> MAUI column at **44 ✅ / 41 🟡 / 0 ⬜**. Its estimate had walked 35–40 → 10–15 → 7–9 → 5–7 → 3–5
> → 2–4 days; the real answer was **0 days of build**.
>
> ### ⚠⚠ THIS PAGE WAS NOT ARCHIVED WHOLE — HALF OF IT IS STILL LIVE ELSEWHERE
>
> `index.md` warned, in advance, that *"archiving them unlifted is how the useful half gets buried
> with the finished half"*. So it was lifted first:
>
> | What was here | Where it lives now | Why it could not be archived |
> |---|---|---|
> | **§0.2 — Matt's rulings**, quoted | [`till-design.md`](../till-design.md) **Part E** | They decide what gets built next, on any till |
> | **§13 — binding defaults** | `till-design.md` **Part E** | Same |
> | **§14 — cached-login horizons** | `till-design.md` **Part E** | Real numbers, still enforced |
> | **§15 — pitfalls that cost a session** | `till-design.md` **Part E** | Each one is a session already lost |
> | **§22 — VAT, settled** | `till-design.md` **Part E** | Explicitly *"not to be re-derived"* |
> | **§8 USER-VERIFY · §17 · §0.3b · §5c · §23 · §1b · §26** | [`MAUI_finaltest.md`](../MAUI_finaltest.md) | The tests, and the record of what hand-runs found |
>
> ⚠ **The section numbers were kept** in `MAUI_finaltest.md` rather than renumbered — other
> documents cite them (*"§5c item 10"*, *"§0.3b"*), and a reference that resolves to the wrong thing
> costs more than a gap in the numbering.
>
> ### Where to go instead
>
> | You want | Open |
> |---|---|
> | What is still unproven on MAUI | [`MAUI_finaltest.md`](../MAUI_finaltest.md) |
> | The steps to run | [`Test Maui.md`](../Test%20Maui.md) |
> | A capability, a money rule, or a ruling | [`till-design.md`](../till-design.md) |
>
> ⚠⚠ **DO NOT REOPEN THIS PAGE FOR NEW WORK.** Five documents became one here at Matt's
> instruction; that one is now done. A new MAUI plan is how the splinter comes back.

---

**Everything below is the document as it stood on 2026-08-23, unedited.**
...
| ✅ | ⚠⚠ **~~The store-gate deadline convention is unpinned~~ — PINNED 2026-08-22, AND FOUR OF SEVEN GATES DID NOT FOLLOW IT** | `TillGateDeadlineTests` | The row said the next caller written without a deadline would restore the fault. It was already worse than that: `PlutusApi`, `PublishedReports`, `CarrierBags` and `Modal` all waited unbounded — the two caches **across a network call**, and Modal's failure means the till can never show a dialog again. ⚠ The pin then found **three more** (`FileOperatorStore`, the OPERATOR ROSTER — a hang there stops anyone signing in) that a hand grep missed, because they wait on `_gate`, not `Gate`. All seven now have a 30s deadline and a decided failure mode: throw (store), the caller's existing "not available" answer (api, caches, roster), or go ahead anyway (modal — a till that cannot ask a question cannot take a payment). ⚠ A `CancellationToken` is NOT a deadline: every one of the four carried `ct`, and nothing cancelled it on the hang path |
...
> ⚠⚠ **SCOPE, SETTLED 2026-08-13 — read [`Loyalty Update across all tills.md`](../To%20do/Loyalty%20Update%20across%20all%20tills.md) before starting this
...
> burn. Full reasoning and the two edge cases in [`Loyalty Update across all tills.md`](../To%20do/Loyalty%20Update%20across%20all%20tills.md) §18. **None of
...
| ⚠⚠ **~~Delete `LoginViewModel.EnsureStoreAsync`~~** | ✅ **DONE 2026-08-23.** ⚠⚠ **AND THE THING THAT BLOCKED IT HAD ALREADY EXPIRED.** The method existed so `App.GetViewModel().Store` was not null, because `StoreInformationViewModel` dereferenced it IN ITS CONSTRUCTOR — which `AppShell` runs while being built, so a correct password threw and the operator was told *"Something went wrong signing in"*. **That constructor was made null-safe on 2026-08-17** and the screen stopped reading the local store at all; the blocker was six days stale when this was checked. ⚠ Every remaining reader already handled null: `Database.cs` guards it, `ReceiptReprint` coalesces it, and both printing paths take it as an argument and cope — *a missing store must not lose a receipt*. ⚠ The two UNGUARDED dereferences (`AddEditViewModel`, `ViewAllViewModel.ExecuteUpdateItemStock` — both unreachable, both L2/L13) now **refuse and log** rather than NullReference. ⚠⚠ **NOT null-coalesced to 0**, exactly as this row warned: no store means no stock row, which is recoverable; a row against store 0 is not. ⚠ Pinned by `LegacyStoreTests` (3, mutation-checked): no unguarded dereference, nobody defaults an id to 0, and the method is really gone — *this row was recorded wrong twice before, and a comment stopped neither time.* |
...
⚠ **That total does NOT include the expanded loyalty programme** ([`Loyalty Update across all tills.md`](../To%20do/Loyalty%20Update%20across%20all%20tills.md),
...
> [`NatApp data translation agent and scripts.md`](../To%20do/NatApp%20data%20translation%20agent%20and%20scripts.md) §7**
