...
| 17.3 | Email/SMS seam + deliverability | ✅ (seam) | Done 2026-07-28 (net10) — the buildable SEAM half; concrete adapter + deliverability dashboard remain gated on a mailer choice. SharedKernel `IMessageSender` + `NullMessageSender` default (accepts nothing → nothing sends). `TenantSendingIdentity` (per tenant+channel from-address/domain — isolation from day one) + `MessageEvent` ledger (sent→bounced/complained, keyed by ProviderMessageId) + `MessageEventStore` + migration. Arch test: core carries no concrete provider (SendGrid/Mailgun/Twilio/SMTP/MailKit… banned in src). Round-trip test: fake adapter send → bounce webhook → event row with tenant attribution + the tenant's from-identity preserved. |
