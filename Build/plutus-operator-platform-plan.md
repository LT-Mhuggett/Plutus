...
| 16.4 | Dunning automation | 🔨 (config built) | Config framework built 2026-07-28 per the operator's "build for each option" decision: `BillingSettings` (GLOBAL single row) + migration; `BillingProviderCatalogue` (**manual / Stripe Billing / Paddle / Chargebee**, field schemas with secret flags); operator endpoints (`/platform/billing/catalogue|config`, secrets write-only) + **Platform → Billing** screen. Default = manual (no automation). The dunning REACTION already exists (billing webhook → status changes, D16 tills keep selling); what remains gated is only the concrete `IBillingProvider` adapter per provider — wire it when an account exists, the selected keys are already stored. Tests: Integration 31 (403 gate, manual default, all 4 in catalogue, secret redaction, unknown provider 400). |
...
| 17.2 | Payment gateway health | 🔨 (per-tenant config built) | Config framework built 2026-07-28 per the operator's decision ("each client might differ; standalone must be an option"): `PaymentGatewaySettings` (TENANT-OWNED, one row per tenant) + migration; `PaymentProviderCatalogue` — **standalone (DEFAULT: external chip & pin, cashier confirms — today's flow made explicit)** + stripe-terminal/sumup/square/adyen/worldpay. Client-facing endpoints (`/payments/gateway` catalogue/get/put — put gated `portal.company.manage`, secrets write-only; `/active` for the till: provider+label+integrated only, never config). Portal: **Card payments** section on the Company tab. Till: checkout shows the setup — standalone hint ("take payment on the chip & pin terminal, confirm approved") or the selected provider with "integration pending" (selling never blocks). Gateway *health monitoring* + concrete terminal integrations remain gated on a wired gateway. Tests: Integration 31 (standalone default, perm gate, secret redaction, cross-tenant isolation). |
