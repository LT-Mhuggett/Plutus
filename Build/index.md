...
In [`To do/`](To%20do/). **Five documents**: the two MAUI plans that lived here were consolidated into
`MAUI-retrofit.md` on 2026-08-12, the loyalty design arrived on 2026-08-13, and the build-shrink
packaging note on 2026-08-18.

⚠ **`Shrink MAUI Build.md` is not a sixth MAUI document** and must not become one. It holds artefact
size and removal order — no capability status, no deploy state, no rulings. If a status claim ever
appears in it, fold it into `MAUI-retrofit.md` and delete it: that is precisely how the five
consolidated documents splintered in the first place.
...
| [Shrink MAUI Build.md](To%20do/Shrink%20MAUI%20Build.md) | **Packaging only — the size of the till artefact and the order to remove things.** Written 2026-08-18 after Matt asked about the publish folder's layout. ⚠ **The visible mess and the real waste are different things**: the 119 language folders are **4.6 MB (1.7%)** and not ours, while **~91 MB (35%)** is Syncfusion + `DocumentFormat.OpenXml` serving screens **no operator can open**. Lowest-risk first: `DocumentFormat.OpenXml` is **15 MB the till never calls** (one `PackageReference` line; the backend uses the other `ExcelHandling` class), then five Syncfusion packages referenced by **zero** files. ⛔ Full Syncfusion removal (76 MB, build a third smaller) is **blocked by L4's ruling** and has a load-bearing ORDER — licence registration last, or a dormant screen becomes a trial-dialog screen. ⚠ **Nothing started, deliberately: do the hand-run on 1.74.0 first**, because XAML type errors here surface at runtime, not at build. ⚠ **Carries no capability status** — `MAUI-retrofit.md` wins on any overlap. |
