...
| [till-subdomain-cutover.md](till-subdomain-cutover.md) | **The out-of-hours runbook for moving the till to `till.plutus…`** and handing the bare host to the landing page (2026-08-25). ⚠ Two steps on purpose — the till is a PWA whose service worker re-caches whatever `/` returns as its **offline shell**, so a one-shot flip would leave tills showing the marketing page when the network drops. Carries the re-enrolment, the tombstone `sw.js`, the verification and the rollback. **Delete it once it has been run.** |
...
| Document | Outcome |
|---|---|
| 🆕 [WP-signup.md](archive/WP-signup.md) | ✅ **Archived 2026-08-23 — built AND deployed the same day it was finished** (backend 1.31.0). All five stages, the operator queue, the client-facing acceptance, all eight DoD lines. ⚠⚠ **Its two unfinished items were LIFTED OUT rather than archived with it** — an open decision inside an archived plan is a decision nobody takes: the **DPA text** went to [WP-landing.md](To%20do/WP-landing.md) §0b (it blocks that plan) and **blocking-vs-nagging** to [plutus-platform-architecture.md](plutus-platform-architecture.md) §12d (it is about tenants that already exist). ⚠ Its own banner read "NOTHING HERE IS IMPLEMENTED" for a day after everything shipped — the lesson is in the archive banner. |
