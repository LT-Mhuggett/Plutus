...
| [`To do/WP-signup.md`](To%20do/WP-signup.md) | **WP-SIGNUP** — self-serve tenancy and the DPA that gates it. ✅ **BUILT AND DEPLOYED 2026-08-23**, all eight DoD lines. ⚠⚠ Signup cannot COMPLETE, deliberately: no DPA is published, because the supplied document names the parties the wrong way round (Leading Talent as **Controller**) and is an unfilled template — §4.3. ⚠ The front door is **closed by default** via the `signup.public` flag (Platform → Flags) — §7b |
| [`To do/WP-landing.md`](To%20do/WP-landing.md) | **WP-LANDING** — the public site. ⚠ **Plan only**, ≈2–3d. A UI over WP-SIGNUP's existing four endpoints, **not a feature** — if it grows endpoints, something has gone wrong. ⚠⚠ Its login is a **link**, never a third auth implementation (architecture §12c) |
