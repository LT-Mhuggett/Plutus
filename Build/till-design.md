...
| Stock as a movement LEDGER (write) | ✅ | ⬜ | `POST /api/v1/stock/movements` (delta) · `POST /api/v1/stock/takes` (absolute) | ⚠ **BLOCKED ON A PERMISSIONS DECISION, NOT ON CODE.** The endpoint is gated on `portal.stock.adjust`, which `RbacSeeder` gives to Owner, Company Admin, Store Manager and the legacy "Stock & Items" role — **Supervisor and Cashier hold NOTHING**, so a supervisor at the counter cannot write off a damaged box. The three ways out are different policies, not implementations; written up in [`maui-whats-left.md`](maui-whats-left.md) §25b for Matt to choose. ⚠ **And it is a LEDGER**: `qty` is a signed DELTA and zero is refused. The only "set it to N" surface is `/stock/takes`, which converts counted − expected server-side — **a screen whose box holds an absolute number must post a TAKE**, or the typed count gets added to the count. |
