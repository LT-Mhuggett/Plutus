...
| **Legacy tax row → VAT band** — `VatBandCache` ↔ `api.ts:786` | ⚠ **divergent on purpose, and MAUI is the strict one** | A tax row claimed by two bands (zero and exempt are both 0%) makes MAUI return **null → unclassified**, which is visible. The web till returns the **first match**, silently attributing takings to whichever sorted first — the exact bug `VatAccounting.BandFor` was changed to stop committing. **The web till should follow, not this relax.** Needs a WP. |
