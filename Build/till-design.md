...
| ⚠ **Finding a past sale AT ALL** (list / search recent sales) | ✅ | ⬜ | `GET /api/v1/sales` — already answers | ⚠ **This is what makes refunds unusable, not the refund rule.** Returns, the refund cap and the read path all shipped (steps 15–17) — but the "Returns" dialog asks for the **original sale ID**, and `TillStore.FindLocalSaleAsync` takes a `Guid` with **no browse, no search, no "today's sales"** anywhere in the app (grep-verified 2026-08-10). The only source of that id is the **barcode on the printed receipt**, so a till with no printer attached cannot refund anything. ⚠ Compounded by discoverability: "Returns" is a **context-menu item on a basket row** — a right-click on Windows, with nothing on screen to suggest it. Needs no platform work; it is a screen, and the same screen **reprint** needs. Rides step 26. |
