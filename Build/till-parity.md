...
| **VAT band on the sale line** (`LineMeta.vatBand`) | ✅ | ⬜ | **WP2c-exempt, 2026-08-08.** ⚠ **MAUI MUST SEND THIS or exempt turnover is silently unrecoverable.** Zero-rated and exempt both declare `vatRateBp = 0`; the band is the only thing that separates them, and it decides whether input tax is recoverable (Notice 706). Resolve it from the item's legacy `TaxId` against each band's published `legacyTaxIds` — `LineMeta.VatBand` already exists in `Plutus.Contracts.Client`. **Leave it null rather than guessing** when the portal hasn't mapped the tax row; the server falls back to snapping the rate and the portal flags the row. A single-purpose gift-card activation/redemption is `"standard"` by the voucher treatment, not by its catalogue row. |
