...
| ⚠ **The integration suite is flaky — and NOT for the reason first recorded** | ⚠⚠ **CORRECTED 2026-08-25.** I first wrote *"flaky under parallelism"*. **That is wrong**: parallelisation was already disabled assembly-wide on 2026-08-11 (`TestParallelism.cs`) for this same `EnsureCreated` symptom. Naming the wrong cause in a gap list is worse than leaving the row blank — the next person tries the fix that is already in place.<br><br>**The real cause, from the exception rather than the shape:** `SQLite Error 1: 'no such table: DpaDocuments'`. `DpaSeedHostedService` is a `BackgroundService` that starts *alongside* schema creation, loses the race, and **retries 10 times at 3-second intervals** — up to 30 seconds. Its own header admits this: *"IT RETRIES, BECAUSE IT RACES THE MIGRATION AND LOST."* In production the migration wins within a second or two. In the test suite, 56 classes each build and dispose a host, so retry loops from disposed hosts outlive their service provider and their in-memory SQLite connection, and the noise lands on whichever class is starting next — which is why a *different* test fails each run.<br><br>**The fix is to remove the race, not to widen the retry:** seed the DPA from `MigrateDatabase`, immediately after the schema is ready, instead of from a competing hosted service. Deterministic, no retry, no lingering background work — and it closes the class of bug the retry comment itself worried about (*"the next new table would hit the same thing"*). ⚠ Must stay non-fatal: a seeder that can stop the backend booting is worse than the thing it seeds.<br><br>⚠ **Why it matters beyond tidiness:** a suite you re-run until it goes green is one where a real failure gets waved through. |
