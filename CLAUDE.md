...
| **Add anything a shop cannot trade without** (a VAT band, a category, a catalogue row, a setting) | ⚠⚠ **Read `plutus-platform-architecture.md` §3.2 — a tenant is BORN complete.** Seed it in `ProvisioningService`, inside the transaction — never in a boot-time sweep, never a backfill. Then **extend the outcome test to DO the new thing**, rather than adding to a list of what to create. ⚠ And if a test of yours seeds something a real tenant would already have, that is a provisioning gap in disguise — fix provisioning instead |
